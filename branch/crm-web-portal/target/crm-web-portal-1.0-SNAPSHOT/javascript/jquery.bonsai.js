	...
	Bonsai.prototype = {
		...
		update : function(){
			...
			this.el.children().each( function(){
				var item = $( this );
				if ( self.options.createCheckboxes )
		...
		serialize : function(){
			...
			return this.el.find( 'li' ).toArray().reduce( function( acc, li ){
				var $li = $( li );
				var state = $li.hasClass( 'expanded' ) ? 'expanded' : $li.hasClass( 'collapsed' ) ? 'collapsed' : null;
		...
		insertCheckbox : function( listItem ){
			if ( listItem.find( '> input[type=checkbox]' ).length )
				return;
			var id = this.checkboxId( listItem ), checkbox = $( '<input type="checkbox" name="' + this.getCheckboxName( listItem ) + '" id="' + id
			...
			checkbox.prop( 'disabled', listItem.data( 'disabled' ) );
			checkbox.attr( 'onclick', listItem.data( 'onclick' ) );
			children.remove();
			listItem.append( checkbox ).append( $( '<label for="' + id + '">' ).append( text ? text : children.first() ) ).append(
		...
		handleDuplicates : function(){
			...
			self.el.on( 'change', 'input[type=checkbox]', function( ev ){
				...
				var selector = 'input[type=checkbox]' + '[value="' + checkbox.val() + '"]'
						+ ( checkbox.attr( 'name' ) ? '[name="' + checkbox.attr( 'name' ) + '"]' : '' )
						+ ( checkbox.prop( 'checked' ) ? ':not(:checked)' : ':checked' );
				self.el.find( selector ).prop( {
					checked : checkbox.prop( 'checked' ), indeterminate : checkbox.prop( 'indeterminate' )
				} ).trigger( 'change' );
			} );
		},
		...
		addSelectAllLink : function(){
			var scope = this.options.scope, self = this;
			function getCheckboxes(){
				// return all checkboxes that are not in hidden list items
				return scope.find( 'li' ).filter( self.options.selectAllExclude || function(){
			...
			} ) ).append( '<i class="separator"></i>' ).append(
					$( '<a class="none">Select none</a>' ).css( 'cursor', 'pointer' ).on( 'click', function(){
						getCheckboxes().prop( {
