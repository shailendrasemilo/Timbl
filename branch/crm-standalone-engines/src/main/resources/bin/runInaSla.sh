...
#find if SLA INA Engine is already running
app_pid=`cat .slaInaEngine.app.pid`
app_run_status=`ps $app_pid | wc -l | tr -d " "`
if [ "$app_run_status" = "2" ]; then
        echo "SLA INA Engine already running!!! PID=" $app_pid
        ...
        mv ../log/crm-sla-ina.log* ${LOG_FOLDER}

        echo "Class Path is : $classpathStr "
        nohup ${JAVA} -mx${MAX_MEMORY_LIMIT}  -cp $classpathStr -Dlog4j.configuration=../config/log4j-sla-ina.xml -Dappconfig.properties=../config/application-properties.properties com.np.tele.crm.sla.engine.SlaInaEngine System 5 1> ../log/crm-sla-ina.log 2> ../log/stderr.log &
        APP_PID=$!
        echo ${APP_PID} > .slaInaEngine.app.pid
        echo "SLA Ina  Engine Process with PID : ${APP_PID}"
        if [[ $? -ne 0 ]]; then
