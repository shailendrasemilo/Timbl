...
if [ "$app_run_status" = "2" ]; then
	...
	nohup ${JAVA} -mx${MAX_MEMORY_LIMIT}  -cp $classpathStr -Dlog4j.configuration=../config/log4j-resolve-ticket.xml com.np.tele.crm.ticket.resolve.engine.ResolveTicketEngine System 5 1> ../log/stdout.log 2> ../log/stderr.log &
	RESOLVE_TICKET_PID=$!
