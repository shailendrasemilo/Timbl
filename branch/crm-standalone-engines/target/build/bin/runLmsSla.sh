...
#find if SLA LMS Engine is already running
app_pid=`cat .slaLmsEngine.app.pid`
app_run_status=`ps $app_pid | wc -l | tr -d " "`
if [ "$app_run_status" = "2" ]; then
    	echo "SLA LMS Engine already running!!! PID=" $app_pid
	...
	mv ../log/crm-sla-lms.log* ${LOG_FOLDER}	

	echo "Class Path is : $classpathStr "
	nohup ${JAVA} -mx${MAX_MEMORY_LIMIT}  -cp $classpathStr -Dlog4j.configuration=../config/log4j-sla-lms.xml -Dappconfig.properties=../config/application-properties.properties com.np.tele.crm.sla.engine.SlaLmsEngine System 5 1> ../log/crm-sla-lms.log 2> ../log/stderr.log &
	APP_PID=$!
	echo ${APP_PID} > .slaLmsEngine.app.pid
	echo "SLA LMS  Engine Process with PID : ${APP_PID}"
	if [[ $? -ne 0 ]]; then
