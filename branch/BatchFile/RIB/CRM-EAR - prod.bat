...
@set MYACCOUNT_EARDIR=%RI_BRANCH_HOME%\crm-selfcare-ear\target\
@echo MYACCOUNT_EARDIR=%MYACCOUNT_EARDIR% 
@set STANDALONE_EARDIR=%RI_BRANCH_HOME%\crm-standalone-engines\target\
@echo STANDALONE_EARDIR=%STANDALONE_EARDIR% 
@set CMS_UPLOAD_EARDIR=%RI_BRANCH_HOME%\crm-cms-upload\target\
...
@cd %PRJDIR%
@echo PRJDIR=%CD%
@Call mvn clean install -Denv=prod -Dclient=ri -Dservicehost=%SERVICE_HOST%
@cd %CLIENT_EARDIR%
